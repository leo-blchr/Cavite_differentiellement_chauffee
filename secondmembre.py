...
def calcul_premier_second_membre_1_T(T, phi, j, dx, dy, dt, Prandtl):
    (N_x, N_y) = np.shape(T)
    ...
    for i in range(1, N_x - 1):  # i = 1 → N_x-2
        premier_membre[i - 1, i - 1] = 1 + dt / (Prandtl * dx * dx)

        second_membre[i - 1] = (
            T[i, j] * (1 - dt / (Prandtl * dy * dy))
            + T[i, j + 1] * ( dt / 8 * (phi[i + 1, j] - phi[i - 1, j]) / (dx * dy) + dt / (2 * Prandtl * dy * dy))
            + T[i, j - 1] * ( -dt / 8 * (phi[i + 1, j] - phi[i - 1, j]) / (dx * dy) + dt / (2 * Prandtl * dy * dy))
        )

        if i < N_x - 2:
            premier_membre[i - 1, i] = dt / 8 * (phi[i, j + 1] - phi[i, j - 1]) / (dx * dy) - dt / (2 * Prandtl * dx * dx)
            premier_membre[i, i - 1] = - dt / 8 * (phi[i, j + 1] - phi[i, j - 1]) / (dx * dy) - dt / (2 * Prandtl * dx * dx)
...
def calcul_premier_second_membre_2_T(T, phi, j, dx, dy, dt, Prandtl):
    (N_x, N_y) = np.shape(T)
    ...
    for i in range(1, N_y-1):  # i = 1 → N_y-2
        premier_membre[i-1, i-1] = 1 + dt/(Prandtl*dy*dy)

        second_membre[i-1] = (
            T[i, j] * (1 - dt / (Prandtl * dx * dx))
            + T[i, j + 1] * (-dt / 8 * (phi[i, j + 1] - phi[i, j - 1]) / (dx * dy) + dt / (2 * Prandtl * dx * dx))
            + T[i, j - 1] * (dt / 8 * (phi[i, j + 1] - phi[i, j - 1]) / (dx * dy) + dt / (2 * Prandtl * dx * dx))
        )

        if i < N_y - 2:
            premier_membre[i - 1, i] = -dt / 8 * (phi[i + 1, j] - phi[i - 1, j]) / (dx * dy) - dt / (2 * Prandtl * dy * dy)
            premier_membre[i, i - 1] =  dt / 8 * (phi[i + 1, j] - phi[i - 1, j]) / (dx * dy) - dt / (2 * Prandtl * dy * dy)
